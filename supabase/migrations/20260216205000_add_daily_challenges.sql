-- Create challenges table for admin-managed quests
create table if not exists public.challenges (
  id uuid default gen_random_uuid() primary key,
  title text not null,
  description text,
  type text check (type in ('daily', 'weekly', 'achievement')) default 'daily',
  goal_type text check (goal_type in ('matches_won', 'max_streak', 'problems_solved', 'login_streak')) default 'matches_won',
